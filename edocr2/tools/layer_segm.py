...
def find_rectangles(img, binary_thres = 127, bin_output_path=None):
    '''Returns a list with rectangles from an image
    Args: img: the image (mechanical engineering drawing)
          binary_thres: threshold value for binary conversion
          bin_output_path: Optional path to save the binary image step
    returns: 
    ...
    # Save the binary image step if path is provided
    if bin_output_path:
        try:
            print(f"    [find_rectangles] Saving binary image step to: {bin_output_path}")
            cv2.imwrite(bin_output_path, thresh)
        except Exception as e:
            print(f"    [find_rectangles] Error saving binary image to {bin_output_path}: {e}")

    # Find contours
    contours, _ = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    print(f"    [find_rectangles] Found {len(contours)} initial contours.") # DEBUG PRINT

    img_boxes = img.copy()
    r = 0
    rect_list=[]
    box_count = 0 # DEBUG COUNT
    for cnt in contours:
    ...
    print(f"    [find_rectangles] Found {box_count} contours passing is_box check and size filter.") # DEBUG PRINT
...
def find_frame(img, frame_thres):
    ...
    print(f"    [find_frame] Found {len(v_peaks)} vertical peaks: {v_peaks}") # DEBUG PRINT
    print(f"    [find_frame] Found {len(h_peaks)} horizontal peaks: {h_peaks}") # DEBUG PRINT
    '''tmp = img.copy()
    ...
    if len(v_peaks) < 2 or len(h_peaks) < 2:
        print("    [find_frame] Not enough peaks found to define a frame.") # DEBUG PRINT
    ...
    def find_best_peaks(peaks, v_h): #v_h is a int= 0 or 1 to identify if we are talking vertical or horizontal peaks
        ...
        if np.any(valid_diff_mask):
            ...
            print(f"    [find_frame] No valid peak pair found for axis {v_h} with min distance {min_frame}") # DEBUG PRINT
            return None # DEBUG RETURN
    
    best_h_peaks = find_best_peaks(h_peaks, 0)
    best_v_peaks = find_best_peaks(v_peaks, 1)

    # DEBUG PRINTS
    if best_h_peaks:
        bottom, top = best_h_peaks
        print(f"    [find_frame] Best horizontal peaks: top={top}, bottom={bottom}")
    else:
        print("    [find_frame] Failed to find best horizontal peak pair.")
        return None
    
    if best_v_peaks:
        right, left = best_v_peaks
        print(f"    [find_frame] Best vertical peaks: left={left}, right={right}")
    else:
        print("    [find_frame] Failed to find best vertical peak pair.")
        return None
...
def segment_img(img, frame_thres = 0.85, autoframe = True, GDT_thres = 0.02, binary_thres = 127, steps_output_path=None):
    
    #Find rectangles
    img_boxes, rect_list, _  = find_rectangles(img, binary_thres= binary_thres, bin_output_path=steps_output_path)
